...
## 1 문자 인식 데이터 만들기
각 문자 영역을 추출했으므로 각 글자를 머신러닝으로 인식시켜 보겠습니다. MNIST의 손글씨 숫자 데이터를 Keras + TensorFlow를 이용하여  학습시켜 보겠습니다. 일단 MNIST의 손글씨 숫자 데이터를 학습하고 가중치 데이터를 저장합니다. 그런다음 저장한 가중치 데이터를 읽어드려 문자를 인식 시켜 보겠습니다. 
...
최근 OS에는 따로 설치하지 않아도 처음부터 굉장히 많은 폰트가 있습니다. 디자인과 관련된 작업을 하는 분이라면 더 많은 폰트가 있을 텐데, OS에 설치돼 있는 모든 폰트를 사용해  숫자 폰트 이미지를 만든 다음 숫자 이미지를 학습시켜 정답률( > 71%)을 높여 보았습니다. 
...
정답률 :  loss = 1.25811360349 , accuracy = 0.920000004768(92%) , baseline error: 8.00%
```

## 결론

여기서 또 재미있는 것은 
내 컴퓨터(개개인의 컴퓨터마다 폰트가 다름)의 OS에 설치돼 있는 모든 폰트를 사용해  숫자 폰트 이미지를 만든 다음 숫자 이미지를 학습시켜 Keras의 mnist의 prediction을 적용한 결과와 Keras의 cnn을 적용한 결과가 같다는 것이다. 

위와 같은 문제의 경우 cnn을 사용하는 것은 추천하지 않는다. 이유는 결과는 같은데 cnn의 경우  상대적으로 cost가 너무 많이 든다.
