...
여기서 또 재미있는 것은 
내 컴퓨터(개개인의 컴퓨터마다 폰트가 다름)의 OS에 설치돼 있는 모든 폰트를 사용해  숫자 폰트 이미지를 만든 다음 숫자 이미지를 학습시켜 Keras의 mnist의 prediction을 적용한 결과와 Keras의 cnn을 적용한 결과가 같다는 것이다. 위와 같은 문제의 경우 cnn을 사용하는 것은 추천하지 않는다.
